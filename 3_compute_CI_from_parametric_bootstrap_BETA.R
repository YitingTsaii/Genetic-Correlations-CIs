...
cover_prob_settingA_beta <- data.frame(true_rho = cover_prob_all_beta$true_rho_k, cover_prob = a) 
save(cover_prob_settingA_beta, file=cover_prob_settingA_beta_path)

b <- apply(CI_width_all_beta[,2:101],1,mean)
CI_width_settingA_beta <- data.frame(true_rho = CI_width_all_beta$true_rho_k, CI_width = b) 
